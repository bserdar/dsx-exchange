...
This folder contains operator-facing configuration only. Internal NATS details (accounts, mappings, leaf nodes) are auto-generated by the `nats-event-bus` Helm chart based on `global.eventBus.clusterType`.

### Key Configuration

Set `global.eventBus.clusterType` to configure the cluster automatically:
...
Auth permissions are defined under `global.eventBus.auth.permissions`:
...
CPC clusters connect to CSC as leaf nodes through the Envoy Gateway (port 7422). Topic filtering and transformation occurs in NATS account configuration, not at the Gateway. Leaf nodes and account configurations are auto-generated based on `global.eventBus.clusterType` and `global.eventBus.crossLayer` settings.
